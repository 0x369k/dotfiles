# Generated by Powerlevel10k configuration wizard on 2024-03-05 at 17:01 CET.
...
function prompt_pyenv() {
  ...
  [[ -n "$pyenv_version" ]] && p10k segment -f 208 -i '⭐' -t "$pyenv_version"
}

function prompt_nb_notebook() {
  local nb_notebook=$(nb status | grep 'Notebook:' | awk '{print $2}')
  if [[ -n "$nb_notebook" ]]; then
    p10k segment -f 208 -i '📓' -t "$nb_notebook"
  fi
}